...
public class CarLandingConfig : MonoBehaviour
{
    ...
    [Tooltip("After landing, continuously bleed sideways velocity toward the car nose for a timed window. Can fight mid-turn; prefer Slip Straighten. Leave off.")]
    [SerializeField] private bool enableLandingLateralBleed = false;
    ...
    [Tooltip("After touchdown (when not steering/drifting), gently ease travel toward the nose with fading authority. Aborts immediately if the player owns the turn.")]
    [SerializeField] private bool enableLandingSlipStraighten = true;
    [Tooltip("How long the soft straighten runs after touchdown.")]
    [SerializeField, Min(0.01f)] private float landingSlipStraightenDuration = 0.3f;
    [Tooltip("Legacy — unused. Straighten now uses a continuous fade (no carry→yank phases).")]
    [SerializeField, Range(0f, 0.95f)] private float landingSlipCarryFraction = 0f;
    ...
    [SerializeField, Range(0f, 1f)] private float landingSlipLateralKeep = 0.75f;
